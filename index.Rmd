...
df_sel <- df %>% filter(substr(who,4,6)=="TUN", rnk < 11)
df_sel<-merge(df_sel,tab_def,by.x="where1",by.y="code")
...
tab4<-res[22:31,c(3,4)]
names(tab4)<-c("El Watan (fr)","pct3")

tab<-cbind(tab1,tab2,tab3,tab4)

kable(tab,digits=c(0,1,1,1,1,1,1,1), row.names = F)
...
# filter elwatan
mat<-mat[,colnames(mat) !="fr_DZA_elwata"]
...
```{r}
cah2 <- HCPC(afc,nb.clust = 7,graph = FALSE,cluster.CA = "columns")
